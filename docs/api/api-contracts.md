...
State lives in `rateLimits/{uid}` (`lastCuratedAt` / `lastDescribedAt` timestamps). The slot is **reserved inside a Firestore transaction before generation** (so two concurrent calls can't both pass — exactly one wins, the other gets `resource-exhausted`), and **rolled back if generation fails** (a server error never burns the user's daily slot). On denial the handler throws `resource-exhausted` with `details.retryAt` (ISO8601 of `lastAt + 24h`) and `details.scope`.
